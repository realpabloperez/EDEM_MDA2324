...
class avgSpeedDoFn(beam.DoFn):
    ...
    def process(self, element):
        ...
        output_dict = {
            ...
            "vehicle_id": key,
            "avg_speed": avg_speed,
            "latitude": payload[-1]['location'][0],
            "longitude": payload[-1]['location'][1]
        }
...
def run():
    ...
    parser.add_argument(
                '--output_bucket',
                required=False,
                help="Bucket para almacenar los datos"
    )
    ...
    with beam.Pipeline(argv=pipeline_opts,options=options) as p:
        ...
        '''(
            processed_data
            | "Write to GCS" >> beam.io.WriteToText(file_path_prefix=args.output_bucket, file_name_suffix=".json")
        )'''
        (
            processed_data.fined_vehicles
                | "Write fined vehicles to BigQuery" >> WriteToBigQuery(
                    table="skilled-seeker-411714:dataflow.coches_multados",
        ...
        (
            processed_data.non_fined_vehicles 
                | "Write non-fined vehicles to BigQuery" >> WriteToBigQuery(
                    table="skilled-seeker-411714:dataflow.coches_sin_multar",
                    create_disposition=beam.io.BigQueryDisposition.CREATE_NEVER,
                    write_disposition=beam.io.BigQueryDisposition.WRITE_APPEND
                )
        )
